...
export default function DriverProfile() {
  ...
  const [rating, setRating] = useState("Not rated yet!");
  const [isAvaliable, setIsAvaliable] = useState(null);
  const [errorLoadingComponent, setErrorLoadingComponent] = useState(null);
  const { user } = useUser();
  const { id } = useParams();

  const onSucces = (data) => {
    setIsAvaliable(data.available);
    setDriver(data);
    if (data.ratings.length > 0) {
      setRating(avarageRating(data.ratings));
    }
  };
              ...
              <div className="flex flex-wrap items-start justify-between mb-2">
                ...
                <div className="flex flex-wrap my-auto items-center">
                  <div className="flex items-center">
                    <span
                      className={`w-[15px] h-[15px] flex rounded-full bottom-0 end-0 -mb-1 -mr-2 border border-white ${
                        isAvaliable ? "bg-green-600" : "bg-red-600"
                      }`}
                    ></span>
                    <span className="text-xs z-10 transition-opacity duration-300 ease-in-out px-3 py-2 whitespace-nowrap text-center transform bg-white rounded-2xl shadow-sm bottom-0 -mb-2 start-full ml-4 font-medium text-secondary-inverse block">
                      {isAvaliable ? "Available" : "Not Available"}{" "}
                    </span>
                  </div>
                </div>
              </div>
